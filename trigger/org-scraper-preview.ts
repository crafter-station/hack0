...
Focus on the main organization name, not event names. Look for "About", "Nosotros", "Quiénes somos" sections.`,
			...
			let errorMessage =
				"Error al extraer datos del sitio web. Verifica que la URL sea correcta y esté accesible.";

			if (error.message?.includes("not currently supported")) {
				errorMessage =
					"Este sitio no está soportado por Firecrawl. Intenta con el sitio web oficial de tu comunidad (evita redes sociales como Instagram, Facebook, Twitter).";
			} else if (error.message?.includes("rate limit")) {
				errorMessage =
					"Límite de scraping alcanzado. Intenta de nuevo en unos minutos.";
			}
		...
		if (!result.json) {
			const errorMessage =
				"No se pudo extraer información del sitio web. Intenta con una URL diferente o completa los campos manualmente.";
			await metadata.set("lastError", errorMessage);
			await metadata.set("status", "error");
			await metadata.set("error", errorMessage);
		...
		const logoUrl = result.metadata?.ogImage
			? extractLogoFromMetadata(
					result.metadata as { ogImage?: string },
					normalizedUrl,
				)
			: extracted.logoUrl;
