...
export async function scrapeDevpostPage(
	url: string,
	...
	try {
		...
		const result = await firecrawl.extract({
					...
					hackathons: {
						...
						items: {
							...
							properties: {
								...
								url: {
									type: "string",
									description: "URL to the hackathon page",
								},
								startDate: {
									type: "string",
								...
								prizeAmount: {
									type: "string",
									description: "Prize amount if available",
								},
								location: {
									type: "string",
			...
			prompt:
				"Extract all hackathons listed on this page. For each hackathon, get the name, description, URL, dates, prize amount, location, themes, logo URL, and organizer name.",
		});
...
export function parseLocation(location: string): {
	...
	if (
		...
		if (
			...
			locationLower.includes("in-person")
		) {
...
export function inferDomains(
	...
	const text =
		`${name} ${description} ${(themes || []).join(" ")}`.toLowerCase();
...
export function inferEventType(name: string, description: string): EventType {
	...
	const typeKeywords: [EventType, string[]][] = [
		...
		[
			"call_for_papers",
			["call for papers", "cfp", "convocatoria de papers", "paper submission"],
		],
		...
		[
			"conference",
			["congreso", "conference", "simposio", "symposium", "cumbre", "summit"],
		],
