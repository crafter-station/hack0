...
export function inferEventType(
  ...
  if (text.includes("meetup") || text.includes("meet up") || text.includes("meet-up")) return "meetup";
  ...
  if (text.includes("bootcamp") || text.includes("boot camp")) return "bootcamp";
...
export function inferCountryFromCity(city?: string): string {
  ...
  const peruCities = ["lima", "arequipa", "cusco", "trujillo", "chiclayo", "piura", "ica", "tacna", "huancayo"];
  if (peruCities.some((c) => cityLower.includes(c))) return "PE";

  const mexicoCities = ["ciudad de méxico", "cdmx", "guadalajara", "monterrey", "puebla", "tijuana"];
  ...
  const brazilCities = ["são paulo", "rio de janeiro", "brasília", "salvador", "fortaleza"];
