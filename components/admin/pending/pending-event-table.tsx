...
function countryFlag(iso: string): string {
	if (!iso || iso.length !== 2) return "\u{1F30E}";
	return iso
		.toUpperCase()
		.split("")
...
const REQUIRED_FIELDS: (keyof Event)[] = [
	"name",
	"startDate",
	"endDate",
	"country",
...
const FIELD_LABELS: Record<string, string> = {
	...
	format: "formato",
	description: "desc",
	eventImageUrl: "imagen",
	websiteUrl: "URL",
	prizePool: "premio",
};

function getMissingCount(event: Event): { missing: number; labels: string[] } {
	const labels: string[] = [];
	...
	return { missing: labels.length, labels };
}

interface PendingEventTableProps {
	events: Event[];
	selected: Set<string>;
...
export function PendingEventTable({
	events,
	selected,
	onSelect,
	onSelectAll,
				...
				<table className="w-full text-sm">
					...
					<tbody className="divide-y">
						{events.map((event) => {
							...
							const { missing, labels } = getMissingCount(event);
								...
								>
									...
									<td className="px-3 py-2.5">
										{missing === 0 ? (
											...
											<span
												className="inline-flex items-center gap-1 rounded-full bg-amber-500/10 px-2 py-0.5 text-xs font-medium text-amber-600 cursor-help"
												title={`Faltan: ${labels.join(", ")}`}
											>
												{missing}
											</span>
										)}
