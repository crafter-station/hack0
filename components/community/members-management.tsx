      ...
      {isEmpty && canManage ? (
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>
              Esta comunidad aún no tiene miembros. Crea una invitación para comenzar a construir tu equipo.
            </EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
        ...
        <>
          ...
          {canManage && (users.length > 0 || activeInvites.length > 0) && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold">
                  Invitaciones activas {activeInvites.length > 0 && `(${activeInvites.length})`}
                </h3>
                        ...
                        <div className="space-y-1 flex-1">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-medium">
                              {invite.roleGranted ? COMMUNITY_ROLE_LABELS[invite.roleGranted] : "Miembro"}
                            </span>
                            <span className="text-xs text-muted-foreground">
                              • {invite.usedCount || 0} uso{(invite.usedCount || 0) !== 1 ? "s" : ""}
                            </span>
                          </div>
                          <p className="text-xs text-muted-foreground">
                            Creada {invite.createdAt ? formatDistanceToNow(new Date(invite.createdAt), { addSuffix: true, locale: es }) : "hace poco"}
                          </p>
                        </div>
                        <div className="flex items-center gap-2">
                          <Button
