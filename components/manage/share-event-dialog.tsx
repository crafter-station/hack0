...
export function ShareEventDialog({ event, children }: ShareEventDialogProps) {
  ...
  const ogImageUrl = event.eventImageUrl || `${process.env.NEXT_PUBLIC_APP_URL || "https://hack0.dev"}/api/og?slug=${event.slug}`;
  ...
  function shareToSocial(platform: string) {
    const text = `${event.name} - ${event.description?.slice(0, 100) || ""}`;
    const urls = {
    ...
    <Dialog>
      ...
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Comparte {event.name} en redes sociales
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          {/* Event Link */}
          <div>
            <label className="text-sm font-medium mb-2 block">Enlace del evento</label>
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <Button
                size="sm"
                variant="outline"
                onClick={() => copyToClipboard(ogImageUrl, "og")}
              >
                {copied === "og" ? (
                  <>
                    <Check className="h-4 w-4 mr-1" />
          ...
          <div>
            <label className="text-sm font-medium mb-2 block">Compartir en redes sociales</label>
