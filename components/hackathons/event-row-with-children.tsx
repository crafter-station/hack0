...
import { BadgeCheck, Sparkles, ChevronDown, ChevronRight, MapPin } from "lucide-react";
...
import { getChildEvents } from "@/lib/actions/hackathons";
import {
  getEventStatus,
  getFormatLabel,
  getEventTypeLabel,
...
export function EventRowWithChildren({ event, categoryConfig }: EventRowWithChildrenProps) {
  const [isExpanded, setIsExpanded] = useState(false);
  const [childEvents, setChildEvents] = useState<Hackathon[]>([]);
...
function ChildEventRow({ event }: { event: Hackathon }) {
  const startDate = event.startDate ? new Date(event.startDate) : null;
  const status = getEventStatus(event);
  const isEnded = status.status === "ended";
  const isOngoing = status.status === "ongoing";
