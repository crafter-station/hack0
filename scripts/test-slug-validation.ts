import { isSlugUnique, slugSchema } from "@/lib/slug-utils";
...
isSlugUnique("hackathon-minedu-2025").then((isUnique) => {
	console.log(
		`  Event "hackathon-minedu-2025" is unique: ${isUnique ? "NO (exists in DB)" : "N/A"}`,
	);

	isSlugUnique("brand-new-unique-slug-99999").then((isUnique2) => {
		console.log(
			`  Event "brand-new-unique-slug-99999" is unique: ${isUnique2 ? "YES (not in DB)" : "NO"}`,
		);
