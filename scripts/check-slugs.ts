...
async function checkSlugs() {
	...
	if (problematic.length > 0) {
		console.log(
			problematic
				.map((e) => `  - ${e.name || "NO NAME"} (slug: ${e.slug || "NO SLUG"})`)
				.join("\n"),
		);
	}

	// Check for spaces in slugs
	const slugsWithSpaces = allEvents.filter((e) => e.slug?.includes(" "));
	...
	const slugsWithUpper = allEvents.filter(
		(e) => e.slug && e.slug !== e.slug.toLowerCase(),
	);
	console.log(`\n❌ Slugs with uppercase: ${slugsWithUpper.length}`);
	if (slugsWithUpper.length > 0) {
		console.log(
			slugsWithUpper
				.map((e) => `  - "${e.slug}" → should be "${e.slug.toLowerCase()}"`)
				.join("\n"),
		);
	}
	...
	const hasIssues =
		...
		slugsWithSpaces.length > 0 ||
		slugsWithUpper.length > 0 ||
		slugsWithSpecial.length > 0 ||
		duplicates.length > 0;
