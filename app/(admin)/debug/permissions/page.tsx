		...
		<div className="min-h-screen bg-background p-8">
			<div className="max-w-2xl mx-auto space-y-6">
				...
				<div className="bg-muted p-4 rounded-lg space-y-2 font-mono text-sm">
					...
					<div>
						<strong>ADMIN_EMAILS env:</strong> {process.env.ADMIN_EMAILS}
					</div>
					<div>
						<strong>Is Admin:</strong> {adminCheck ? "✅ YES" : "❌ NO"}
					</div>
				</div>

				<div className="bg-muted p-4 rounded-lg space-y-2 font-mono text-sm">
					<div>
						<strong>Test Event:</strong> {testSlug}
					</div>
					<div>
						<strong>Event Found:</strong> {event ? "✅ YES" : "❌ NO"}
					</div>
					<div>
						<strong>Organization ID:</strong> {event?.organizationId || "None"}
					</div>
					<div>
						<strong>Membership Found:</strong> {membership ? "✅ YES" : "❌ NO"}
					</div>
					{membership && (
						<div>
							<strong>Membership Role:</strong> {membership.role}
						</div>
					)}
					<div>
						<strong>Can Manage:</strong> {canManage ? "✅ YES" : "❌ NO"}
					</div>
				</div>
				...
				{!canManage && (
					<div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">
						...
						<ul className="list-disc ml-5 mt-2">
							...
							<li>
								OR have membership in organization {event?.organizationId}
							</li>
						</ul>
					</div>
				)}
