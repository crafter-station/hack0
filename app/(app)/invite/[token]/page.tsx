...
import { COMMUNITY_ROLE_LABELS, communityMembers } from "@/lib/db/schema";
...
async function InviteContent({ token }: { token: string }) {
				...
				<div className="rounded-lg border bg-card p-8 space-y-6">
					<div className="text-center space-y-4">
						...
						<div className="space-y-2">
							<h1 className="text-2xl font-bold">
								Invitación a{" "}
								{invite.community.displayName || invite.community.name}
							</h1>
					...
					<div className="rounded-lg bg-muted p-4 space-y-2">
						...
						<p className="text-lg font-semibold">
							{COMMUNITY_ROLE_LABELS[invite.roleGranted ?? "follower"]}
						</p>
					</div>
					...
					<div className="text-center space-y-4">
						...
						<div className="space-y-2">
							...
							<p className="text-sm text-muted-foreground">
								...
								{invite.community.displayName || invite.community.name}. Esta
								invitación no es necesaria.
							</p>
						</div>
					</div>

					<Button asChild className="w-full">
						<a href={`/c/${invite.community.slug}`}>Ir a la comunidad</a>
					</Button>
				</div>
			</div>
		);
	...
	if (existingMembership) {
		const existingRoleLevel = roleHierarchy[existingMembership.role ?? "follower"] || 0;
					...
					<div className="rounded-lg border bg-card p-8 space-y-6">
						<div className="text-center space-y-4">
							...
							<div className="space-y-2">
								...
								<p className="text-sm text-muted-foreground">
									Ya eres {COMMUNITY_ROLE_LABELS[existingMembership.role ?? "follower"]} de{" "}
									{invite.community.displayName || invite.community.name}. Esta
									invitación no es necesaria.
								</p>
							</div>
			...
			<div className="max-w-md mx-auto mt-20">
				<div className="rounded-lg border bg-card p-8 space-y-6">
					<div className="text-center space-y-4">
						...
						<div className="space-y-2">
							<h1 className="text-2xl font-bold">Actualización de rol</h1>
							<p className="text-sm text-muted-foreground">
								Te invitaron a un rol superior en{" "}
								{invite.community.displayName || invite.community.name}
							</p>
						</div>
					</div>
							...
							<div className="text-center">
								...
								<div className="px-3 py-1.5 rounded-md bg-background border">
									<p className="text-sm font-medium">
										{COMMUNITY_ROLE_LABELS[existingMembership.role ?? "follower"]}
									</p>
								</div>
							</div>
					...
					<form
						action={async () => {
							...
							if (result.success && result.community) {
								...
								redirect(
									`/invite/${token}?error=${encodeURIComponent(result.error || "Error desconocido")}`,
								);
							}
						}}
					>
						<AcceptInviteButton
							label="Aceptar actualización"
